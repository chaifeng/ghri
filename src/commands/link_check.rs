...
use crate::runtime::{is_path_under, Runtime};
...
pub fn check_versioned_links_for_version<R: Runtime>(
    ...
    let filtered: Vec<_> = links.iter().filter(|l| l.version == version).cloned().collect();
...
mod tests {
    ...
    fn test_check_links_categorizes_correctly() {
        ...
        let links = vec![
            LinkRule { dest: valid_dest, path: None },
            LinkRule { dest: invalid_dest, path: None },
        ];
